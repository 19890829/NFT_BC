...
class Blockchain(object):
    ...
    def generate_genesis_block(self, genesisblock):
        address = {'Address': 'Alice', 'Weight': 64, 'Age': 0}
...
def main():
    """Run test"""
    account = {'Address': 'Alice', 'Weight': 64}
    account2 = {'Address': 'Bob', 'Weight': 64}
    account3 = {'Address': 'Carl', 'Weight': 64}
    account4 = {'Address': 'Dan', 'Weight': 80}
    
    
    blockchain = Blockchain(GENESIS_BLOCK, account)
    blockchain.generate_new_block(52)#52
    ...
    for x in range(0, 995):
        ...
        accountx = {'Address': str(x), 'Weight': randint(50, 99)}
        DATE = datetime.now()
        GENESIS_BLOCKX = {
            "Index": 0,
            "Timestamp": str(DATE),
        ...
        BC = Blockchain(GENESIS_BLOCKX, accountx)
        for i in clients:
            BC.add_new_node(i)
        BC.pos()
        clients.append(BC)
